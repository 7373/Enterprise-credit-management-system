require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'common/busUtil', 'handlebars', 'jquery','jquery.serialize','laydate','common/validateRules', 'common/ajaxfileupload'], function (layer, dataTable, util, http, busUtil,handlebars) {
    ...
    $fileBox=$("#fileBoxSQS"),idStr;
    
    filesArrHT = removeStr($('#filesHT').val().split("|"));
    filesArrSQS = removeStr($('#filesSQS').val().split("|"));
    filesArrDLR = removeStr($('#filesDLR').val().split("|"));
    filesArrQT = removeStr($('#filesQT').val().split("|"));
    
    var num = $('#contractTypeCode').val().substring(0,$('#contractTypeCode').val().indexOf("-"))
    init();
    ...
    function initPreSelect(){
    	if(num == '')
    		...
    		busUtil.editPreFcfType('contractTypeFir', num);
    }
    /**
     * 初始化合同类别子下拉框*/
    function initSonSelect(){
    	if(num == '')
    		...
    		busUtil.editSonFcfType('contractTypeSec', num,$('#contractTypeCode').val());
    }
    
    $('#contractTypeFir').change(function(){
    	$('#contractTypeSec').html('');
    	busUtil.editSonFcfType('contractTypeSec',$('#contractTypeFir').val(),$('#contractTypeCode').val());
    });
    	...
    	$('#hxForm').validate({
            ...
            submitHandler: function () {
	            	...
	            	if(g.test(contractName)){
	            		layer.msg('合同名称输入格式不正确，请不要输入字符',{time:3000});
	            		return ;
	            	}
