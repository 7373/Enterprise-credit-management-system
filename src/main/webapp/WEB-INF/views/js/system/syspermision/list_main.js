require(['common/util', 'component/iframeLayer', 'common/http', 'handlebars', 'layer1', 'ztree'], function (util, layer, http, handlebars) {
    ...
    function ztreeInit() {
        ...
        http.httpRequest({
            type: 'post',
            url: '/admin/system/syspermision/list.json',
    ...
    function showPermision(id, parentNode) {
        ...
        http.httpRequest({
            type: 'post',
            url: '/admin/system/syspermision/query',
            data: {id: id},
            success: function (data) {
                if (data.status == 'success') {
        ...
        util.bindEvents([{
            ...
            handler: function () {
                ...
                layer.dialog({
                    title: '新增权限',
                    area: ['628px', '434px'],
                    content: '/admin/system/syspermision/show?sid=' + sid,
            ...
            handler: function () {
                ...
                layer.dialog({
                    title: '修改权限',
                    area: ['628px', '434px'],
                    content: '/admin/system/syspermision/show?id=' + id,
                    callback: function (data) {
                        //重新加载列表数据
                        if (data.reload) {
                            ztreeInit();
            ...
            handler: function () {
                ...
                http.httpRequest({
                    type: 'post',
                    url: '/admin/system/syspermision/delete',
                    data: {id: id},
                    success: function (data) {
                        if (data.status == 'success') {
                            if (data.msg) {
            ...
            handler:function () {
                http.httpRequest({
                    type:"GET",
                    url: '/admin/system/syspermision/reload',
                    success:function (data) {
                        var errorMsg = data.msg;
                        var errors = data.errors;
                        if (errors.length > 0) {
